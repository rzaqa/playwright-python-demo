import allure

from playwright.sync_api import Page, expect


class BasePage:
    """Base class for all page objects."""

    def __init__(self, page: Page):
        self.page = page
        self.URL = None
        self._errors = []

    @allure.step("Open a given path relative to base_url.")
    def open(self, path: str = "/"):
        self.page.goto(path)

    # --- Soft assertion helper ---
    def soft_check(self, name: str, fn):
        """Execute a check and collect any assertion error instead of stopping the test."""
        try:
            fn()
        except Exception as e:
            self._errors.append(f"{name}: {str(e)}")

    def verify_soft_assertions(self):
        """Raise aggregated AssertionError if any soft check failed."""
        if self._errors:
            msg = "❌ UI element verification failed:\n" + "\n".join(self._errors)
            raise AssertionError(msg)
        else:
            print("✅ All soft checks passed successfully.")

    @allure.step("Verify that the current page has the expected URL.")
    def verify_page_loaded_by_url(self, expected_url=None, timeout=10000):
        url_to_check = expected_url or self.URL
        if not url_to_check:
            raise ValueError("Expected URL not provided or defined in the page object.")
        with allure.step(f"Verify page loaded by URL: {url_to_check}"):
            expect(self.page).to_have_url(url_to_check, timeout=timeout)

    def expect_visible(self, locator):
        expect(locator).to_be_visible()

    def get_title(self):
        return self.page.title()



